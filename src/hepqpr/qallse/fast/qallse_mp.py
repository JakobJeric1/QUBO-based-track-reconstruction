...
class QallseMp(Qallse):
    ...
    def _find_max_path(self, qplet: Quadruplet, direction=0):
        # Use recursion to compute the max path (longest chain) for this qplet.
        # PERF: memoize results because the same qplet is visited many times.

        cache = self._max_path_cache
        ...
        if direction <= 0 and qplet.t1.inner:
            # compute inner max
            if len(qplet.t1.inner) == 1:
                inner_length = self._find_max_path(qplet.t1.inner[0], direction=-1)
            else:
                inner_length = max(self._find_max_path(q, direction=-1) for q in qplet.t1.inner)

        if direction >= 0 and qplet.t2.outer:
            # compute outer max
            if len(qplet.t2.outer) == 1:
                outer_length = self._find_max_path(qplet.t2.outer[0], direction=1)
            else:
                outer_length = max(self._find_max_path(q, direction=1) for q in qplet.t2.outer)

        res = 1 + inner_length + outer_length
        cache[key] = res
        return res
    ...
    def build_model(self, *args, **kwargs):
        ...
        # filter quadruplets
        self._max_path_cache = {}  # PERF: reset cache for this build
