...
from .utils import truth_to_xplets, diff_rows
...
class DataWrapper:
    ...
    @staticmethod
    def _get_dkey(h1: int, h2: int) -> int:
        # Unique and fast key for an ordered pair (h1,h2).
        # TrackML hit_id fits in 32 bits, so we can pack into one int.
    ...
    def is_real_doublet(self, doublet: TDoublet) -> XpletType:
        ...
        h1, h2 = doublet
        return self._lookup.get(self._get_dkey(h1, h2), XpletType.FAKE)
    ...
    def is_real_xplet(self, xplet: TXplet) -> XpletType:
        ...
        get_key = self._get_dkey

        h1 = xplet[0]
        h2 = xplet[1]
        t0 = lookup_get(get_key(h1, h2), XpletType.FAKE)
        if t0 == XpletType.FAKE:
            return XpletType.FAKE

        # all consecutive doublets must have the same type
        for i in range(1, n - 1):
            h1 = xplet[i]
            h2 = xplet[i + 1]
            ti = lookup_get(get_key(h1, h2), XpletType.FAKE)
            if ti != t0:
                return XpletType.FAKE
    ...
    def sample_qubo(self, Q: TQubo) -> TDimodSample:
        ...
        for (k1, k2), v in Q.items():
            ...
            # cache parsed names -> list[int]
            hits = name_cache.get(k1)
            if hits is None:
                # faster than list(map(int, ...)) because it avoids Python map overhead
                hits = [int(x) for x in k1.split('_')]
                name_cache[k1] = hits

            # inline is_real_xplet (avoids function call overhead)
            h1, h2 = hits[0], hits[1]
            t0 = lookup(get_key(h1, h2), XpletType.FAKE)
            if t0 == XpletType.FAKE:
                sample[k1] = 0
                continue
            ...
            for i in range(1, len(hits) - 1):
                h1 = hits[i]
                h2 = hits[i + 1]
                ti = lookup(get_key(h1, h2), XpletType.FAKE)
                if ti != t0:
                    ok = False
                    break

            sample[k1] = 1 if ok else 0
