...
class Volayer:
    ...
    _index = {vl: i for i, vl in enumerate(ordering)}  # PERF: O(1) lookup

    @classmethod
    def get_index(cls, volayer: Tuple[int, int]) -> int:
        return cls._index[tuple(volayer)]

    @classmethod
    def difference(cls, volayer1, volayer2) -> int:
        return cls._index[tuple(volayer2)] - cls._index[tuple(volayer1)]
...
class Triplet(Xplet):
    ...
    def __init__(self, d1: Doublet, d2: Doublet, curvature_val=None, drz_val=None):
