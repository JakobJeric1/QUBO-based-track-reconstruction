...
def solve_sqa(Q, num_reads=200, seed=None):
    ...
    if seed is None:
        ...
        return sampler.sample_qubo(Q, num_reads=num_reads, seed=seed)
...
if op.exists(OUT_FILE):
    i = 1
    while True:
        new_file = op.join(OUT_DIR, f"solve_qubos_sqa({i}).csv")
...
with tqdm(total=len(EVENTS)*len(DSS), desc="Solving QUBOs (OpenJij SQA)", unit="job") as pbar:
    for event, ds in product(EVENTS, DSS):
        ...
        with time_this() as tinfo:
            response = solve_sqa(Q, num_reads=200)
